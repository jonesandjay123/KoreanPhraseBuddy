...
- 主畫面顯示名稱為 `Seoul Buddy`，App 內標題為 `Seoul Phrase Buddy`
- 已加入自訂 App icon
- 輸入中文句子並新增成小卡
- 用 SwiftUI `List` 顯示小卡
...
`.xcconfig` 裡的 build setting 名稱是 `GEMINI_API_KEY`，但 App 在 runtime 讀的是產生後 Info.plist 裡的 `GeminiAPIKey`：

```swift
Bundle.main.object(forInfoDictionaryKey: "GeminiAPIKey")
```

所以 target build setting 需要包含：

```text
INFOPLIST_KEY_GeminiAPIKey = $(GEMINI_API_KEY)
```

或是在 Xcode 的 target Info 頁面確認有這個 custom property：

- Key: `GeminiAPIKey`
- Type: `String`
- Value: `$(GEMINI_API_KEY)`

這是 iOS 版本最容易混淆的地方：`GEMINI_API_KEY` 是 build setting，`GeminiAPIKey` 是 App runtime 讀取的 Info key。
...
如果 iPad 主畫面仍顯示舊名稱或舊 icon，通常是 iOS cache。請先從裝置刪掉 App，再用 Xcode 重新安裝。
...
5. 加入 iOS 原生語音輸入：Jones 預演版用中文 `zh-TW`；櫻井妹妹課堂版改成日文 `ja-JP`
6. 加韓文欄位與假翻譯
7. 加複製韓文
8. 加韓文播放 TTS
9. 最後才接 Gemini
